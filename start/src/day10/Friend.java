...
public class Friend {
	/*
	 	...
	 	친구의 정보는
	 	이 클래스를 객체로 만든 후 
	 	변수의 내용을 변경을 시켜서 기억시키면 된다.
	 	
	 	이때
	 	변수의 경우 아무나 이 객체의 변수에 접근해서 데이터를 변경시키면 안되므로
	 	변수는 감춰두고 변수에 접근할 수 잇는 함수들을 만들어서 데이터를 처리한다.
	 	
	 */
	
	private String name, addr, tel, mail, birth, sTest;
	private int age;
	...
	private char gen; // 성별을 기억할 변수 남자는 'M' 여자는 'F'로 기억하기로
	
		/*this.변수명 : 현재 실행중인 객체 자기자신의 주소를 기억하는 변수
