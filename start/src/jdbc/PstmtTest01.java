...
public class PstmtTest01 {
	...
	public PstmtTest01() {
		...
		try {
			...
			// 이 명령이 정상적으로 실행되면 오라클에서 세션하나를 확보하는
			// 3. 질의명령 준비하고
			//String sql = "SELECT empno eno, ename, job, mar, to_char(hiredate, 'YYYY/MM/DD') hdate, deptno dno from emp where ";
			...
			/*
				...
				이럴때 사용하는 명령전달 도구로 
				PreparedStatement가 있다.
			 */
			pstmt = con.prepareStatement(sql);
			// 명령전달 도구는 준비가 되어있지만
			// 도구안에 세팅된 질의명령은 아직 완벽한 질의명령은 아니다.
			// 따라서 질의명령을 완벽하게 만들어 줘야 한다.
			// 완벽한 질의명령으로 만드는 방법은 ? 에 정확한 데이터를 채워주면 된다.
			/*
			 	불완전한 질의명령(?가 있는 질의문)에 데이터를 채우는 방법
			...
			while(rs.next()) {
				//rs.next() ==> 작업실행줄을 한줄 내려주는 함수. 이때 내린결과 꺼내올 데이터가 있으면 true를 반환해준다
				// 없으면 false... 
